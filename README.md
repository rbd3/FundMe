# Foundry Fund Me
This Solidity project, built with Foundry, is a decentralized funding contract called FundMe. It allows users to send Ether to the contract, which is then tracked and mapped to their addresses. The contract ensures a minimum contribution of 5 USD (converted from Ether using Chainlink's price feed) and includes gas-optimized features like immutable variables and efficient state resets. Only the contract owner can withdraw funds, and the contract supports both receive and fallback functions for seamless Ether transfers. This project demonstrates best practices in Solidity development, including error handling, modifiers, and gas optimization techniques.
...
```
git clone https://github.com/rbd3/FundMe.git
cd FundMe
make
```
